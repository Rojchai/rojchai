...
//+------------------------------------------------------------------+
// 🔥 ENTRY (จับแตะจริง ไม่พลาดจังหวะ)
void CheckEntry(double line, OrderState &state)
{
   ...
   // BUY: อยู่บน แล้วลงมาแตะ
   if(prevClose > line && low <= line + buffer)
   {
      OpenOrder(true, state);
   }

   // SELL: อยู่ล่าง แล้วขึ้นมาแตะ
   if(prevClose < line && high >= line - buffer)
   {
...
//+------------------------------------------------------------------+
// 🔥 SMART CUT (ใช้ M15 ตามที่คุณเลือก)
